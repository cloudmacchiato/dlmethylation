...
def preprocessing2 (path_fn, gene_fn, meth_fn, label_fn):
    ### pathway datasets
    pathway = pd.read_csv(path_fn, header=0)       
    print(">> Pathway Data :",path_fn)
    pathway_info = pathway.iloc[:,1:]
    ...
    #print dimension of sample and number of genes
    print("sample_dim : ",sample_dim, sample_dim2)
    print("input_size (number of genes, methylation sites): ",input_dim, input_dim2)
    ...
    patient = list(data.iloc[:,1:].columns.values.tolist()) 
    print("patient list : ",patient[0:6])
    print("feature list : ",features[0:6],features_meth[0:6])
